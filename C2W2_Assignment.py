...
# * 모듈 또는 클래스 내부용 변수나 함수에서 사용을 지칭 -- 이런것은 외부에서 사용안하길 바람
...
# tfx -> components -> CsvExampleGen(input_base)
                                      
# Run the component using the InteractiveContext instance
context.run(example_gen)
# 앞서 선언한 context instance에 example_gen 을 넣고 구동 
### END CODE HERE
...
def get_records(dataset, num_records):
    ...
    for tfrecord in dataset.take(num_records):
        ...
        example = tf.train.Example()  # 모듈 초기화하면 한다 ;; 
...
# 앞서 선언한 example_gen.outputs 매쏘드를 사용하여 examples를 넣고 돌린다
# Run the component
context.run(statistics_gen)
### END CODE HERE
